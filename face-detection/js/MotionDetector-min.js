...
var MotionDetector = (function () {
    ...
    if (l) {
      ...
      for (var g = a.data.length, b = 0; b < g; )
        (a.data[b] = 0.5 * (255 - a.data[b]) + 0.5 * f[e].data[b]),
          (a.data[b + 1] = 0.5 * (255 - a.data[b + 1]) + 0.5 * f[e].data[b + 1]),
          (a.data[b + 2] = 0.5 * (255 - a.data[b + 2]) + 0.5 * f[e].data[b + 2]),
          (a.data[b + 3] = 255),
          (b += 4);
  ...
  return {
    init: function () {
      navigator.getUserMedia ? navigator.getUserMedia({ video: !0 }, g, m) : console.error('Your browser does not support getUserMedia');
